...
> Current smaller models (≤ 27B parameters) are not able to call the available tools sequentially—they attempt to call both tools at the same time, which leads to incorrect results.  
> If you use a larger LLM (more than 27B parameters), you can combine both tools in a single AI Data Processing node, as larger models can more reliably call tools one after the other.
